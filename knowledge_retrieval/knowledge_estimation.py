...
from .torch_io import load_trusted_torch_artifact
...
class KnowledgeEstimation:
    def __init__(self, task, buffer_size=10, base_db_path=None):
        self.task = task
    ...
    def prepare_data(self, dataset_name):
        ...
        if os.path.exists(graph_pt):
            # Load processed data
            data_obj = load_trusted_torch_artifact(graph_pt, map_location="cpu")
            return data_obj
        else:
    ...
    def get_dataset_path(self, dataset_name):
        return str(self.base_db_path / dataset_name.lower())
    ...
    def load_pretrained_model(self, dataset_name, data_obj):
        ...
        if os.path.exists(model_path):
            ...
            best_model = load_trusted_torch_artifact(model_path, map_location=self.device)
        else:
            best_emd = float('inf')
            best_model = None
